...
resource "aws_lambda_function" "compute_queue_backlog" {
  ...
  role          = var.execution_role_arn != "" ? var.execution_role_arn : element(concat(aws_iam_role.compute_queue_backlog.*.arn, [""]), 0)
...
resource "aws_iam_role_policy" "compute_queue_backlog" {
  ...
  role   = element(concat(aws_iam_role.compute_queue_backlog.*.name, [""]), 0)
